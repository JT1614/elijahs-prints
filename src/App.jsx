...
function AdminPanel({ products, onSave, onLogout, orders, onUpdateOrders, onSaveFilaments, onSaveCategories, categoryMeta, onSaveCategoryMeta, autoBadges }) {
 ...
 const [editingColour, setEditingColour] = useState(null);
  ...
  const [showBulkReprice, setShowBulkReprice] = useState(false);
  const [bulkRepriceOverrides, setBulkRepriceOverrides] = useState({});
...
{ id: "colours", label: "🎨 Colours", shortLabel: "🎨 Colours", count: ALL_COLORS.length },
      ...
      {adminTab === "products" && (<>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16, gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {(() => {
            const repriceEligible = products.filter(p => {
              const cats = getProductCategories(p);
              return cats.some(c => (categoryMeta[c] || {}).audience === "adult") && !cats.includes("Dragons");
            });
            if (repriceEligible.length === 0) return null;
            return (
              <Tooltip position="bottom" text={`Bulk reprice adult boxed products using the floor + £0.50 formula. ${repriceEligible.length} eligible products (Planters, Household, Bird Feeders). Dragons excluded.`}>
                <button onClick={() => { setBulkRepriceOverrides({}); setShowBulkReprice(true); }} style={{ padding: "10px 20px", borderRadius: 10, border: "1px solid rgba(16,185,129,0.3)", background: "rgba(16,185,129,0.08)", color: "#10b981", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: S.fontHead }}>💰 Bulk Reprice ({repriceEligible.length})</button>
              </Tooltip>
            );
          })()}
          {(() => {
...
{adminTab === "categories" && (
      ...
      {/* Bulk Reprice Modal */}
      {showBulkReprice && (() => {
        const PACKAGING_COST = 0.96;
        const MARGIN_PCT = 0.40;
        const calcNewPrice = (grams) => {
          const floor = ((grams || 0) * 0.01 + PACKAGING_COST) / (1 - MARGIN_PCT);
          const target = floor + 0.50;
        ...
        const eligible = products.filter(p => {
          ...
          const formulaPrice = calcNewPrice(p.grams);
          const newPrice = bulkRepriceOverrides[p.id] !== undefined ? bulkRepriceOverrides[p.id] : formulaPrice;
          const change = newPrice - (p.price || 0);
          const margin = newPrice > 0 ? ((newPrice - (p.grams || 0) * 0.01 - PACKAGING_COST) / newPrice * 100) : 0;
          return { ...p, formulaPrice, newPrice, change, margin };
        }).sort((a, b) => {
          const catA = getProductCategories(a).join(", ");
          const catB = getProductCategories(b).join(", ");
          return catA.localeCompare(catB) || (a.price || 0) - (b.price || 0);
        });
        const increases = eligible.filter(p => p.change > 0);
        const decreases = eligible.filter(p => p.change < 0);
        const unchanged = eligible.filter(p => p.change === 0);
        const currentTotal = eligible.reduce((s, p) => s + (p.price || 0), 0);
        const newTotal = eligible.reduce((s, p) => s + p.newPrice, 0);
        const uplift = currentTotal > 0 ? ((newTotal - currentTotal) / currentTotal * 100) : 0;
        let lastCat = "";

        const applyAll = async () => {
          if (!window.confirm(`Apply new prices to ${eligible.length} products?\n\n${increases.length} increases, ${decreases.length} decreases, ${unchanged.length} unchanged.\n\nRevenue: £${currentTotal.toFixed(2)} → £${newTotal.toFixed(2)} (+£${(newTotal - currentTotal).toFixed(2)})`)) return;
          try {
            const priceMap = {};
            eligible.forEach(p => { priceMap[p.id] = p.newPrice; });
            const updatedProducts = products.map(p => priceMap[p.id] !== undefined ? { ...p, price: priceMap[p.id] } : p);
            await saveProducts(updatedProducts);
            setProducts(updatedProducts);
            setSavedMsg("Prices updated!"); setTimeout(() => setSavedMsg(""), 3000);
            setShowBulkReprice(false);
          } catch (err) {
        ...
        return (
          <div style={{ position: "fixed", inset: 0, zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
            <div onClick={() => setShowBulkReprice(false)} style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)" }} />
            <div style={{ position: "relative", width: "min(720px, 100%)", maxHeight: "90vh", overflow: "auto", background: "#151530", border: `1px solid ${S.border}`, borderRadius: 20, padding: 28 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <h3 style={{ fontSize: 18, fontWeight: 800, fontFamily: S.fontHead, color: S.text, margin: 0 }}>💰 Bulk Reprice — Adult Boxed Products</h3>
                <button onClick={() => setShowBulkReprice(false)} style={{ background: "rgba(255,255,255,0.05)", border: `1px solid ${S.border}`, color: "#aaa", width: 36, height: 36, borderRadius: "50%", cursor: "pointer", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
              </div>

              <div style={{ fontSize: 11, color: S.dimmer, marginBottom: 12, lineHeight: 1.5 }}>
                Formula: (grams × £0.01 + £0.96 packaging) ÷ (1 − 40%) + £0.50, rounded up to nearest 50p.<br/>
                Scope: Planters, Household, Bird Feeders. Dragons excluded. Click any new price to override.
              </div>

              {/* Summary cards */}
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 8, marginBottom: 16 }}>
                {[
                  { label: "Products", value: eligible.length, color: S.teal },
                  { label: "Increases", value: increases.length, color: "#10b981" },
                  { label: "Decreases", value: decreases.length, color: decreases.length > 0 ? "#ef4444" : S.dimmer },
                  { label: "Unchanged", value: unchanged.length, color: S.dimmer },
                  { label: "Current", value: `£${currentTotal.toFixed(0)}`, color: S.muted },
                  { label: "New", value: `£${newTotal.toFixed(0)}`, color: "#10b981" },
                  { label: "Uplift", value: `+${uplift.toFixed(1)}%`, color: "#10b981" },
                ].map((c, i) => (
                  <div key={i} style={{ padding: "8px 10px", borderRadius: 10, background: "rgba(255,255,255,0.03)", border: `1px solid ${S.border}`, textAlign: "center" }}>
                    <div style={{ fontSize: 10, color: S.dimmer, fontFamily: S.fontHead, fontWeight: 600, marginBottom: 2 }}>{c.label}</div>
                    <div style={{ fontSize: 16, color: c.color, fontWeight: 800, fontFamily: S.fontMono }}>{c.value}</div>
                  </div>
                ))}
              </div>

              {/* Product table */}
              <div style={{ maxHeight: 400, overflowY: "auto", borderRadius: 10, border: `1px solid ${S.border}` }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: "rgba(255,255,255,0.04)", position: "sticky", top: 0 }}>
                      {["Product", "g", "Current", "New", "Change", "Margin"].map(h => (
                        <th key={h} style={{ padding: "8px 8px", textAlign: h === "Product" ? "left" : "right", color: S.muted, fontFamily: S.fontHead, fontWeight: 700, fontSize: 10, borderBottom: `1px solid ${S.border}`, whiteSpace: "nowrap" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {eligible.map((p, idx) => {
                      const cat = getProductCategories(p).join(", ");
                      const showHeader = cat !== lastCat;
                      lastCat = cat;
                      const changeColor = p.change > 0 ? "#10b981" : p.change < 0 ? "#ef4444" : S.dimmer;
                      const isOverridden = bulkRepriceOverrides[p.id] !== undefined;
                      return (
                        <React.Fragment key={p.id}>
                          {showHeader && (
                            <tr><td colSpan={6} style={{ padding: "8px 8px 4px", fontWeight: 800, color: S.teal, fontFamily: S.fontHead, fontSize: 11, borderTop: idx > 0 ? `1px solid ${S.border}` : "none", background: "rgba(0,201,167,0.04)" }}>{cat}</td></tr>
                          )}
                          <tr style={{ background: idx % 2 === 0 ? "transparent" : "rgba(255,255,255,0.015)" }}>
                            <td style={{ padding: "6px 8px", color: S.text, fontWeight: 600, maxWidth: 220, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</td>
                            <td style={{ padding: "6px 8px", textAlign: "right", color: S.dimmer, fontFamily: S.fontMono }}>{p.grams || 0}</td>
                            <td style={{ padding: "6px 8px", textAlign: "right", color: S.muted, fontFamily: S.fontMono }}>£{(p.price || 0).toFixed(2)}</td>
                            <td style={{ padding: "6px 8px", textAlign: "right" }}>
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  if (!isNaN(v)) setBulkRepriceOverrides(prev => ({ ...prev, [p.id]: v }));
                                }}
                                style={{ width: 60, textAlign: "right", background: isOverridden ? "rgba(132,94,247,0.12)" : "rgba(255,255,255,0.06)", border: isOverridden ? "1px solid rgba(132,94,247,0.4)" : `1px solid ${S.border}`, borderRadius: 4, color: "#10b981", fontFamily: S.fontMono, fontWeight: 700, fontSize: 12, padding: "3px 6px" }}
                              />
                            </td>
                            <td style={{ padding: "6px 8px", textAlign: "right", color: changeColor, fontFamily: S.fontMono, fontWeight: 700 }}>
                              {p.change > 0 ? `+£${p.change.toFixed(2)}` : p.change < 0 ? `£${p.change.toFixed(2)}` : "—"}
                            </td>
                            <td style={{ padding: "6px 8px", textAlign: "right", color: p.margin >= 45 ? "#10b981" : p.margin >= 35 ? "#f59e0b" : "#ef4444", fontFamily: S.fontMono }}>{p.margin.toFixed(0)}%</td>
                          </tr>
                        </React.Fragment>
                      );
                    })}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16 }}>
                <div style={{ fontSize: 11, color: S.dimmer }}>
                  {Object.keys(bulkRepriceOverrides).length > 0 && (
                    <button onClick={() => setBulkRepriceOverrides({})} style={{ background: "none", border: "none", color: S.purple, cursor: "pointer", fontSize: 11, fontFamily: S.fontHead, fontWeight: 600, padding: 0 }}>Reset all overrides</button>
                  )}
                </div>
                <button onClick={applyAll} style={{ padding: "12px 28px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #10b981, #059669)", color: "#fff", fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: S.fontHead, boxShadow: "0 4px 16px rgba(16,185,129,0.3)" }}>
                  ✅ Apply All Prices
                </button>
              </div>
            </div>
